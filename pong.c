...
void loop() {
  ...
  if ((ball.x < paddle_1.x + paddle_1.w) && (ball.y - ball.h >= paddle_1.y) &&
      (ball.y <= paddle_1.y + paddle_1.h)) {
    ball_x_direction = 1;
  }

  if ((ball.x + ball.w >= paddle_2.x) && (ball.y - ball.h >= paddle_2.y) &&
      (ball.y <= paddle_2.y + paddle_2.h)) {
    ball_x_direction = -1;
  }
